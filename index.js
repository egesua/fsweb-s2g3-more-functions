...
function ortalamaBul(array1) {
  if (array1.length == 0) {
    return null
  } else {
    const avrg = array1.reduce((total , num) => total + num, 0) / array1.length;
...
function ortalamadanBuyukleriBul(arr1, callback) {
  if (arr1.length == 0) {
    return null;
  }
  const bigger = [];
  const smaller = [];
  const total = callback(arr1);
